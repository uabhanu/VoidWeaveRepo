namespace Systems
{
    ...
    public partial struct DashJob : IJobEntity
    {
        ...
        private void Execute(in BaseMoveSpeedComponent baseMoveSpeedComponent , ref DashCooldownComponent dashCooldownComponent , ref DashDurationComponent dashDurationComponent , in DashMultiplierComponent dashMultiplierComponent , ref MoveSpeedComponent moveSpeedComponent , in PlayerInputComponent playerInputComponent)
        {
            ...
            // PlayerInput & 16 corresponds to the Dash input bit defined in InputSystem
            dashDurationComponent.Duration = math.select(math.max(0f , dashDurationComponent.Duration - DeltaTime) , DurationMax , (playerInputComponent.PlayerInput & 16) != 0 && dashCooldownComponent.Timer <= 0f);
            dashCooldownComponent.Timer = math.select(math.max(0f , dashCooldownComponent.Timer - DeltaTime) , CooldownMax , (playerInputComponent.PlayerInput & 16) != 0 && dashCooldownComponent.Timer <= 0f);
